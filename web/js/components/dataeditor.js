...
export class DataEditor extends HTMLElement {
  ...
  get columns() { return this._columnDefs.slice(); }
  /** @param {Array} cols */
  set columns(cols) {
  ...
  get items() { return this._cloneItems(); }
  set items(arr) {
  ...
  get quickAdd() { return this._quickAddConfig; }
  set quickAdd(cfg) {
  ...
  clear() { this._rowData = []; this._renderBody(); this._emitItemsChanged(); }
  ...
  _applyColumnDefaults(obj) {
    ...
    this._columnDefs.forEach(col => {
      if (!(col.key in out)) {
        ...
        else if (Object.prototype.hasOwnProperty.call(col, 'default')) out[col.key] = col.default;
  ...
  _cloneItems() { return this._rowData.map(r => ({ ...r })); }
  ...
  _renderQuickAdd() {
    ...
    // SVG icon
    this._quickAddContainerEl.appendChild(document.createRange().createContextualFragment(quickAddSvg));
    // Quick Add buttons
    items.forEach(name => {
      const btn = document.createElement('button');
      btn.type = 'button';
      ...
      btn.addEventListener('click', () => {
        ...
        if (typeof cfg.onAdd === 'function') obj = cfg.onAdd(name);
          ...
          const key = (this._columnDefs[0] && this._columnDefs[0].key) || 'value';
          obj = { [key]: name };
        }
        this.addItem(obj || {});
      });
  ...
  _installDnD() {
    ...
    this._tbodyEl.addEventListener('dragstart', e => {
      ...
      try { e.dataTransfer.setData('text/plain', String(this._dragFromIndex)); } catch { }
    ...
    this._tbodyEl.addEventListener('dragover', e => {
      ...
      if (insertionIndex === this._dragFromIndex || insertionIndex === this._dragFromIndex + 1) {
  ...
  _clearDragVisuals() {
    Array.from(this._tbodyEl.querySelectorAll('.drag-over-before, .drag-over-after, .dragging')).forEach(tr => {
