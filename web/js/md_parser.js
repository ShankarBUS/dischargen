...
export function parseMarkdown(md) {
  ...
  function closeLists(toLevel = -1) {
    while (listStack.length && listStack[listStack.length - 1].level > toLevel) {
      listStack.pop();
    }
  }

  function currentList() { return listStack[listStack.length - 1]; }
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    if (m) {
      ...
      while (j < rawLines.length) {
        ...
        const contIndent = next.match(/^(\s*)/)[1].replace(/\t/g, '    ').length;
      ...
      if (textPart.trim()) item.blocks.push({ type: 'paragraph', runs: paragraphRunsFromText(textPart.trim()) });
      continue;
    }
...
export function parseInline(text) {
  const runs = [];
  let i = 0; let bold = false; let italic = false;
  while (i < text.length) {
