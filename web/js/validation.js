...
export function validateField(input) {
  ...
  if (cfg.required) {
    if ((input.type === 'checkbox' && !input.checked) || v === '') { valid = false; msg = 'Required'; }
  }
  
  if (valid && cfg.min !== undefined && input.type === 'number') {
    if (parseFloat(v) < parseFloat(cfg.min)) { valid = false; msg = `Min ${cfg.min}`; }
  }
  
  if (valid && cfg.max !== undefined && input.type === 'number') {
    if (parseFloat(v) > parseFloat(cfg.max)) { valid = false; msg = `Max ${cfg.max}`; }
  }

  if (valid && cfg.pattern) {
    try { const r = new RegExp(cfg.pattern); if (!r.test(v)) { valid = false; msg = 'Invalid format'; } } catch (e) { /* ignore */ }
  }
...
export function validateAll(container) {
  ...
  let all = true;
  inputs.forEach(inp => { if (!validateField(inp)) all = false; });
  return all;
}
